...
const ConditionGameScreen = ({ }) => {
  ...
  const onNextCard = async () => {
    setLoading(true);
    if (text?.is_condition_specification_test) {
      const checkResult = await checkUserSelection(text.id, userSentenceSpecifications, 'condition');
      if (!checkResult.isValid) {
        ...
        if (checkResult.testSpecifications.length > 0) {
          messageHeader = "Oups, raté! Voilà les conditions qu'il fallait trouver :";
        } else {
